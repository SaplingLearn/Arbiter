      ...
      if (method === "POST") {
        switch (tail) {
          ...
          case "findings": {
             ...
             *
             * Accepting an extraction is a single decision a reviewer makes about a list,
             * and posting it finding by finding would write nine `evidence_added` entries
             * into the hash chain for it - a record that reads like nine separate acts of
             * judgement instead of the one that happened. It also reuses the same checks
             * as the single path rather than growing a second, laxer one: each finding in
             * the array passes `findingProblem`, and one that fails rejects the whole
             * request rather than being skipped, so a reviewer never accepts ten and
             * silently gets nine.
             */
            if (Array.isArray(body)) {
              ...
              for (const one of many) {
                const bad = await findingProblem(one, caseId, deps);
                if (bad !== null) return json(res, 400, { error: "bad_request", detail: bad });
              }
              const r = await deps.service.addFindings(caseId, many);
              return r.ok
                ? json(res, 201, { ...r.value })
                : json(res, ERROR_STATUS[r.error.kind] ?? 400, r.error);
            }
            const f = body as CoveringFinding;
            /* THE SAME CHECKS AS THE BULK PATH, and from the same function. A second,
               inline copy here is how the strict path stops being the rule the moment
               one of them is edited - see `findingProblem`. */
            const problem = await findingProblem(f, caseId, deps);
          ...
          case "extract": {
            ...
            const inputs = await deps.service.extractionInputs(caseId);
 ...
 * location is.
 */
async function findingProblem(f: CoveringFinding, caseId: string, deps: ServerDeps): Promise<string | null> {
