...
const svc = async (participants: string[]): Promise<DeliberationService> => {
...
const seed = async (s: DeliberationService, actor = "owner") =>
...
describe("the prepared fixture matches the document it claims", () => {
   ...
   */
  it("names the sha256 of the file it was transcribed from", async () => {
   ...
   */
  it("covers all six consequence questions with findings that assert", async () => {
    const s = await svc(["ann", "bea", "cal"]);
    await seed(s);
    const inv = (await s.inventory("c1"))!;
  ...
  it("leaves the mechanism questions a regulatory review cannot answer absent", async () => {
    const s = await svc(["ann"]);
    await seed(s);
    const inv = (await s.inventory("c1"))!;
...
describe("seeding a case from a recognised document", () => {
  it("adds every finding and seals a position for all but one seat", async () => {
    const s = await svc(["ann", "bea", "cal"]);
    const r = await seed(s, "owner");
   ...
   */
  it("holds the reveal until the open seat answers", async () => {
    const s = await svc(["ann", "bea", "cal"]);
    await seed(s);
    expect((await s.reveal("c1", "owner", "2026-08-16T11:00:00Z", "all_in")).ok).toBe(false);

    const p: Position = {
      participantId: "ann", call: "cannot_conclude", reasoning: "Mine.",
      citedFindingIds: [], external: [], submittedAt: "2026-08-16T11:05:00Z",
    };
    expect((await s.submit("c1", p)).ok).toBe(true);
    expect((await s.reveal("c1", "owner", "2026-08-16T11:10:00Z", "all_in")).ok).toBe(true);
  });

  it("holds open the uploader's own seat when they are on the panel", async () => {
    const s = await svc(["ann", "bea", "cal"]);
    const r = await seed(s, "ann");
  ...
  it("writes demo_seeded into the chain ahead of what it seeded", async () => {
    ...
    const kinds = (await store.entries("c1")).map((e) => e.kind);
    ...
    expect(verifyChain(await store.entries("c1"))).toEqual([]);
    ...
    const entry = (await store.entries("c1")).find((e) => e.kind === "demo_seeded")!;
  ...
  it("adds nothing once somebody has answered, and says why", async () => {
    ...
    await s.submit("c1", { ...mine, participantId: "bea", submittedAt: "2026-08-16T11:00:00Z" });
    await s.submit("c1", { ...mine, participantId: "cal", submittedAt: "2026-08-16T11:00:00Z" });
    expect((await s.reveal("c1", "owner", "2026-08-16T11:10:00Z", "all_in")).ok).toBe(true);
    const anns = (await s.view("c1", "owner"))?.revealed?.find((p) => p.participantId === "ann");
    expect(anns?.reasoning).toBe("My own words.");
  });

  it("recognises nothing for bytes it has never seen", async () => {
