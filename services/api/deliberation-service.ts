...
export class DeliberationService {
   ...
   * participant.
   *
   * BEST EFFORT, AND IT REPORTS WHAT IT DID. Every step is a normal call through the same
   * guards a person's typing goes through - `addFindings` still refuses once somebody has
   * answered, `submit` still refuses a citation to a finding that does not exist - so a
   * case already part-way through is topped up rather than overwritten, and a refused
   * step is counted rather than thrown. An upload must never fail because the convenience
   * attached to it could not run.
   *
   * THE LOG ENTRY GOES FIRST. If seeding half-succeeds the record still says a fixture
   * was applied and names it. Writing the entry last would let a partial seed that
   * crashed look like hand-typed evidence.
   *
   * ONE SEAT IS LEFT OPEN, AND THAT IS THE WHOLE DESIGN. The point is not a finished case
   * - it is removing the wait for colleagues who are not in the room. Every participant
   * but one is given a reading and the remaining seat stays empty for the person driving.
   * They write their own position, and only then does the reveal unlock, because the
   * server holds it until everybody has answered. A fixture that filled every seat would
   * skip the one stage the product is most about.
   *
   * WHICH SEAT: the uploader's when they are on the panel, since they are at the
   * keyboard; otherwise the first, which is the top row of the roster everywhere it is
   * drawn. Roster order is not the order the emails were typed in, so "the last one"
   * would mean looking it up and "the one at the top" does not.
   *
   * SEALED SEQUENTIALLY, not with Promise.all: `submit` reads the case, appends, and
   * writes it back, so two of them in flight against one case race on that read and the
   * second overwrites the first's position. The loop is the correctness, not the style.
   */
  async seedFromFixture(
    ...
    const roster = (await this.store.getCase(caseId))?.participantIds ?? [];
    // The uploader's seat when they hold one, otherwise the first. `?? null` rather than
    // a bare index so an empty roster leaves nothing open rather than undefined.
    const leftOpenFor = roster.includes(actorId) ? actorId : roster[0] ?? null;

    let positionsSealed = 0;
    const seats = roster.filter((participantId) => participantId !== leftOpenFor);
    for (const [i, participantId] of seats.entries()) {
      const p = fixture.positions[i % fixture.positions.length];
      if (p === undefined) continue;
      const r = await this.submit(caseId, { ...p, participantId, submittedAt: at });
   ...
   *
   * `addFinding` re-publishes on every call - it appends a fresh `case_opened` (which is
   * where the findings list actually lives) and a fresh `inventory_published` - so adding
   * nine one at a time writes eighteen entries before anybody has said anything. Typed by
   * hand that is a fair record of nine separate edits. Arriving together in one document
   * it is not: it is one act, and the Record stage rendered it as twenty rows of noise
   * above the events a reader came to see.
   *
   ...
   */
  async addFindings(
    caseId: string, findings: CoveringFinding[],
  ): Promise<Result<{ inventory: Inventory; added: string[]; duplicates: string[] }>> {
    const guard = await this.evidenceGuard(caseId);
    if (!guard.ok) return guard;
    ...
    if (added.length === 0) {
      ...
      return { ok: true, value: { inventory: inv!, added: [], duplicates } };
    }
    return {
      ok: true,
      value: {
        inventory: await this.republish(caseId, [...current, ...added], guard.value),
   ...
   * covered. The modality is the service's to answer for the same reason: it is read off
   * the `case_opened` entry, and nothing outside here should be parsing the log for it.
   */
  async extractionInputs(
    caseId: string,
  ): Promise<{ checklist: EvidenceChecklist; modality: Modality } | null> {
    if (await this.store.getCase(caseId) === null) return null;
    return { checklist: this.checklist, modality: await this.modalityOf(caseId) };
  }
