...
export class DeliberationService {
  ...
  async view(caseId: string, viewerId: string): Promise<BlindView | null> {
    const c = await this.store.getCase(caseId);
  ...
  async adjudicate(caseId: string, adjudication: unknown, at: string, actorId: string): Promise<Result<DeliberationCase>> {
    const c = await this.store.getCase(caseId);
