...
| **Reveal & verdict** | Unreachable until everyone has answered. Then the split, the disagreement analysis, the AI adjudication, and the report - anyone named on the case can print the whole record as a PDF (`GET /api/cases/:id/report`, `?format=html` for the markup it is printed from) |
